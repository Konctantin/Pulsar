...
function Director:Test(item)
    if type(item) == 'table' then
        for i, v in ipairs(item) do
            if T.Condition:Run(v[2]) and self:Test(v[1]) then
                return true
            end
        end
    elseif type(item) == 'string' then
        if T.Action:Test(item) then
            print(format('Test: |cffffff00%s|r', item or "-"))
            return true
        end
    end
end
...
function Director:Action(item)
    if type(item) == 'table' then
        for i, v in ipairs(item) do
            if T.Condition:Run(v[2]) and self:Action(v[1]) then
                return true
            end
        end
    elseif type(item) == 'string' then
        if T.Action:Run(item) then
            return true
        end
    else
        error('No item')
...
local function CheckCondition(item)
    if type(item) == 'string' then
        return T.Condition:ParseCondition(item)
    ...
    elseif not item then
        ...
        T.assert(false, 'Invalid Script (Struct Error)')
    end
end

local function CheckAction(item)
    if type(item) == 'string' then
        return T.Action:ParseAction(item)
    elseif type(item) == 'table' then
        for i, v in ipairs(item) do
            CheckCondition(v[2])
            CheckAction(v[1])
...
function Director:Select()
    ...
    for name, plugin in T:IterateEnabledPlugins() do
